...
namespace __magic {
    ...
    /* Values copied from https://github.com/GunshipPenguin/shallow-blue/blob/master/src/attacks.h */

const u64 rookMagicPerSq[64] = {
...
static std::array<u8, 64> toShift(u8* indexBits) {
    std::array<u8, 64> arr;
    for (int i = 0; i < 64; i++) {
        arr[i] = 64 - indexBits[i];
    }

    return arr;
}

extern const std::array<u8, 64> rookMagicShift = toShift((u8*)&rookIndexBits);
extern const std::array<u8, 64> bishopMagicShift = toShift((u8*)&bishopIndexBits); 
